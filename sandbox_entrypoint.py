...
def _copy_runtime_inputs_to_scratch():
    """Copy read-only runtime inputs into the writable scratch directory."""
    scratch = Path.cwd()
    for name in ["fortify_policy.json"]:
        src = Path("/app") / name
        dst = scratch / name
        if not src.exists():
            raise FileNotFoundError(f"required sandbox input missing: {src}")
        if not dst.exists():
...
if __name__ == "__main__":
    ...
    _copy_runtime_inputs_to_scratch()
