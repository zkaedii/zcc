...
Type *parse_declarator_inner(Compiler *cc, Type *base, char *name_out);
...
Type *parse_declarator(Compiler *cc, Type *base, char *name_out) {
    Type *type = parse_declarator_inner(cc, base, name_out);

    /* CG-PARSE-001: Consume optional GCC __asm__("name") rename.
     * Phase B1 implementation: parse and discard.
     * Full rename honor lives in Phase B2 (codegen).
     * Spec: C99 §6.7 + GCC extension.
     * Supports string concatenation: __asm__("" "foo" "bar").
     */
    if (cc->tk == TK_ASM) {
        next_token(cc);         /* consume TK_ASM */
        expect(cc, TK_LPAREN);  /* consume '(' */
        while (cc->tk == TK_STR) {
            /* Discard — do not concatenate or store */
            next_token(cc);
        }
...
static Node *parse_func_def(Compiler *cc, Type *ret_type, char *name, int is_static) {
    ...
    /* CG-PARSE-001: Consume optional GCC __asm__("name") rename for primary function symbols.
     * Phase B1: parse and discard. (codegen unpatched).
     */
    if (cc->tk == TK_ASM) {
        next_token(cc);
        expect(cc, TK_LPAREN);
...
Node *parse_program(Compiler *cc) {
    ...
    while (cc->tk != TK_EOF) {
        ...
        /* CG-PARSE-001: Consume optional GCC __asm__("name") rename for primary global variable/typedef symbols.
         * Phase B1: parse and discard. (codegen unpatched).
         */
        if (cc->tk == TK_ASM) {
            next_token(cc);
