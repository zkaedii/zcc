...
def run_pipeline(ir_data, max_iterations=10):
    ...
    # Import strength reduction (lazy to avoid circular deps)
    try:
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        # Pass 3.5: Strength Reduction (MUL→SHL, DIV→SHR, MOD→AND)
        if has_sr:
            n = strength_reduction(functions)
            cumulative['strength_reduction'] += n
            iter_changes += n
...
def main():
    ...
    print(f"    {'-'*66}")
    for s in scores[:10]:
        print(f"    {s['function']:<40} {s['instructions']:>6} {s['h0_base']:>8.1f} {s['prime_energy']:>8.1f}")
    print()
