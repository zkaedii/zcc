...
async def run_zcc(
    ...
    cmd = [zcc_binary] + source_files + ["-o", "tmp_forge_out.s"]
    print(_c(CYAN, f"🔧 ZCC: ") + " ".join(cmd))
    print(_c(DIM, f"   ZCC_EMIT_IR=1 ZCC_IR_BACKEND=1 timeout={timeout}s"))
...
class LocalModelClient:
    ...
    def _build_payload(self, system: str, user: str, stream: bool) -> tuple[str, dict]:
...
class MutationForge:
    ...
    async def _mutate_function(
        ...
        try:
            ...
            # Best-effort: extract whatever JSON object is present
            start = clean.find("{")
            end   = clean.rfind("}")
    ...
    if args.score_only:
        ...
        if args.output:
            with open(args.output, "w") as f:
                for i, fn in enumerate(targets, 1):
                    # Write JSON compatible score object directly from the dataclass
                    f.write(json.dumps(fn.to_dict()) + "\n")
                    
        for i, fn in enumerate(targets[:15], 1):
