...
static void test_t81_eq_wide(void) {
    ir_module_t *mod;
    evm_lifter_t ls;
    evm_lift_result_t res;
    ...
    TEST("T81: EQ with wide known constants evaluates correctly");
    mod = new_module();
...
/* ── T82: ISZERO with wide known constants ───────────────────────── */
static void test_t82_iszero_wide(void) {
    ir_module_t *mod;
    evm_lifter_t ls;
    evm_lift_result_t res;
    ...
    TEST("T82: ISZERO with wide known constants evaluates correctly");
    mod = new_module();
...
/* ── T83: LT/GT with wide known constants ────────────────────────── */
static void test_t83_lt_gt_wide(void) {
    ir_module_t *mod;
    evm_lifter_t ls;
    evm_lift_result_t res;
    ...
    TEST("T83: LT/GT with wide known constants evaluates correctly");
    mod = new_module();
...
/* ── T84: SLT/SGT with wide known constants ────────────────────────── */
static void test_t84_slt_sgt_wide(void) {
    ir_module_t *mod;
    evm_lifter_t ls;
    evm_lift_result_t res;
    /* PUSH32 (-1), PUSH32 (1), SLT */
    static const unsigned char bc_slt_neg_pos[] = {
        0x7f,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, /* a = -1 */
        0x7f,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    /* PUSH32 (1), PUSH32 (-1), SGT */
    static const unsigned char bc_sgt_pos_neg[] = {
        0x7f,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ...
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, /* b = -1 */
        0x13 /* SGT */
    };
    /* PUSH32 (-2), PUSH32 (-1), SLT */
    static const unsigned char bc_slt_neg_neg[] = {
        0x7f,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, /* a = -2 */
        0x7f,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    ...
    /* PUSH32 (0x80..00), PUSH32 (0x7f..ff), SLT */
    static const unsigned char bc_slt_boundary[] = {
        0x7f,
        0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, /* a = min neg */
        0x7f,
        0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, /* b = max pos */
        0x12 /* SLT */
    };
    
    TEST("T84: SLT/SGT with wide known constants evaluates correctly");
    
    mod = new_module();
    lift_bytes(&ls, mod, bc_slt_neg_pos, 67);
    res = evm_lift_bytecode(&ls);
    CHECK(res == EVM_LIFT_OK, "SLT wide -1 < 1: lift OK");
    CHECK(ls.stack.state[0] == EVM_VAL_KNOWN_NARROW, "SLT wide -1 < 1: yields narrow known");
    CHECK(ls.stack.const_vals[0] == 1, "SLT wide -1 < 1: 1 (true)");
    evm_lifter_destroy(&ls);
    ir_module_free(mod);

    mod = new_module();
    lift_bytes(&ls, mod, bc_sgt_pos_neg, 67);
    res = evm_lift_bytecode(&ls);
    CHECK(res == EVM_LIFT_OK, "SGT wide 1 > -1: lift OK");
    CHECK(ls.stack.state[0] == EVM_VAL_KNOWN_NARROW, "SGT wide 1 > -1: yields narrow known");
    CHECK(ls.stack.const_vals[0] == 1, "SGT wide 1 > -1: 1 (true)");
    evm_lifter_destroy(&ls);
    ir_module_free(mod);

    mod = new_module();
    lift_bytes(&ls, mod, bc_slt_neg_neg, 67);
    res = evm_lift_bytecode(&ls);
    CHECK(res == EVM_LIFT_OK, "SLT wide -2 < -1: lift OK");
    CHECK(ls.stack.state[0] == EVM_VAL_KNOWN_NARROW, "SLT wide -2 < -1: yields narrow known");
    CHECK(ls.stack.const_vals[0] == 1, "SLT wide -2 < -1: 1 (true)");
    evm_lifter_destroy(&ls);
    ir_module_free(mod);

    mod = new_module();
    lift_bytes(&ls, mod, bc_slt_boundary, 67);
    res = evm_lift_bytecode(&ls);
    CHECK(res == EVM_LIFT_OK, "SLT wide min_neg < max_pos: lift OK");
    CHECK(ls.stack.state[0] == EVM_VAL_KNOWN_NARROW, "SLT wide min_neg < max_pos: yields narrow known");
    CHECK(ls.stack.const_vals[0] == 1, "SLT wide min_neg < max_pos: 1 (true)");
    evm_lifter_destroy(&ls);
    ir_module_free(mod);
}

/* ── T85: ADD/SUB exact modular propagation ────────────────────────── */
static void test_t85_add_sub_wide(void) {
    ir_module_t *mod;
    evm_lifter_t ls;
    evm_lift_result_t res;
    
    /* PUSH32 (1), PUSH32 (2), ADD */
    static const unsigned char bc_add_simple[] = {
        0x7f,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    /* PUSH32 (3), PUSH32 (1), SUB */
    static const unsigned char bc_sub_simple[] = {
        0x7f,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    /* PUSH32 (-1), PUSH32 (1), ADD */
    static const unsigned char bc_add_wrap[] = {
        0x7f,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    ...
    /* PUSH32 (0), PUSH32 (1), SUB */
    static const unsigned char bc_sub_wrap[] = {
        0x7f,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    TEST("T85: ADD/SUB exact modular propagation");

    mod = new_module();
    lift_bytes(&ls, mod, bc_add_simple, 67);
    res = evm_lift_bytecode(&ls);
    CHECK(res == EVM_LIFT_OK, "ADD wide 1 + 2: lift OK");
    CHECK(ls.stack.state[0] == EVM_VAL_KNOWN_WIDE, "ADD wide 1 + 2: yields wide known");
    CHECK(ls.stack.const_vals[0] == 3, "ADD wide 1 + 2: 3");
    evm_lifter_destroy(&ls);
    ir_module_free(mod);

    mod = new_module();
    lift_bytes(&ls, mod, bc_sub_simple, 67);
    res = evm_lift_bytecode(&ls);
    CHECK(res == EVM_LIFT_OK, "SUB wide 3 - 1: lift OK");
    CHECK(ls.stack.state[0] == EVM_VAL_KNOWN_WIDE, "SUB wide 3 - 1: yields wide known");
    CHECK(ls.stack.const_vals[0] == 2, "SUB wide 3 - 1: 2");
    evm_lifter_destroy(&ls);
    ir_module_free(mod);

    mod = new_module();
    lift_bytes(&ls, mod, bc_add_wrap, 67);
    res = evm_lift_bytecode(&ls);
    CHECK(res == EVM_LIFT_OK, "ADD wide max + 1: lift OK");
    CHECK(ls.stack.state[0] == EVM_VAL_KNOWN_WIDE, "ADD wide max + 1: yields wide known");
    CHECK(ls.stack.const_vals[0] == 0, "ADD wide max + 1: 0");
    CHECK(ls.stack.wide_vals[0].bytes[31] == 0, "ADD wide max + 1: wide low byte 0");
    evm_lifter_destroy(&ls);
    ir_module_free(mod);

    mod = new_module();
    lift_bytes(&ls, mod, bc_sub_wrap, 67);
    res = evm_lift_bytecode(&ls);
    CHECK(res == EVM_LIFT_OK, "SUB wide 0 - 1: lift OK");
    CHECK(ls.stack.state[0] == EVM_VAL_KNOWN_WIDE, "SUB wide 0 - 1: yields wide known");
    CHECK(ls.stack.const_vals[0] == -1, "SUB wide 0 - 1: -1 in narrow space");
    CHECK(ls.stack.wide_vals[0].bytes[31] == 0xff, "SUB wide 0 - 1: wide low byte 0xff");
    evm_lifter_destroy(&ls);
    ir_module_free(mod);
}

/* ── T86: MUL exact modular propagation ────────────────────────── */
static void test_t86_mul_wide(void) {
    ir_module_t *mod;
    evm_lifter_t ls;
    evm_lift_result_t res;
    
    /* PUSH32 (2), PUSH32 (3), MUL */
    static const unsigned char bc_mul_simple[] = {
        0x7f,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    /* PUSH32 (0), PUSH32 (7), MUL */
    static const unsigned char bc_mul_zero[] = {
        0x7f,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    /* PUSH32 (1), PUSH32 (9), MUL */
    static const unsigned char bc_mul_ident[] = {
        0x7f,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    /* PUSH32 (max_u256), PUSH32 (max_u256), MUL
       (-1) * (-1) = 1 modulo 2^256 */
    static const unsigned char bc_mul_wrap[] = {
        0x7f,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    ...
    mod = new_module();
    lift_bytes(&ls, mod, bc_mul_simple, 67);
    res = evm_lift_bytecode(&ls);
    CHECK(res == EVM_LIFT_OK, "MUL wide 2 * 3: lift OK");
    CHECK(ls.stack.state[0] == EVM_VAL_KNOWN_WIDE, "MUL wide 2 * 3: yields wide known");
    CHECK(ls.stack.const_vals[0] == 6, "MUL wide 2 * 3: 6");
    evm_lifter_destroy(&ls);
    ir_module_free(mod);

    mod = new_module();
    lift_bytes(&ls, mod, bc_mul_zero, 67);
    res = evm_lift_bytecode(&ls);
    CHECK(res == EVM_LIFT_OK, "MUL wide 0 * 7: lift OK");
    CHECK(ls.stack.state[0] == EVM_VAL_KNOWN_WIDE, "MUL wide 0 * 7: yields wide known");
    CHECK(ls.stack.const_vals[0] == 0, "MUL wide 0 * 7: 0");
    evm_lifter_destroy(&ls);
    ir_module_free(mod);

    mod = new_module();
    lift_bytes(&ls, mod, bc_mul_ident, 67);
    res = evm_lift_bytecode(&ls);
    CHECK(res == EVM_LIFT_OK, "MUL wide 1 * 9: lift OK");
    CHECK(ls.stack.state[0] == EVM_VAL_KNOWN_WIDE, "MUL wide 1 * 9: yields wide known");
    CHECK(ls.stack.const_vals[0] == 9, "MUL wide 1 * 9: 9");
    evm_lifter_destroy(&ls);
    ir_module_free(mod);

    mod = new_module();
    lift_bytes(&ls, mod, bc_mul_wrap, 67);
    res = evm_lift_bytecode(&ls);
    CHECK(res == EVM_LIFT_OK, "MUL wide -1 * -1: lift OK");
