...
class CodeGen:
    ...
    def _type_of(self, node: Node) -> CType:
        ...
        if isinstance(node, BinOp):
            if node.ty and node.ty != T_INT: return node.ty
            lty = self._type_of(node.left)
            rty = self._type_of(node.right)
            if node.op in ('+','-') and lty.kind == TypeKind.PTR: return lty
            if node.op == '+' and rty.kind == TypeKind.PTR: return rty
            if node.op in ('==','!=','<','>','<=','>=','&&','||'): return T_INT
            if lty.kind in (TypeKind.LONG, TypeKind.ULONG) or rty.kind in (TypeKind.LONG, TypeKind.ULONG): return T_LONG
            return lty
        if isinstance(node, UnaryOp):
            if node.ty and node.ty != T_INT: return node.ty
            inner = self._type_of(node.expr)
            if node.op in ('post++','post--','pre++','pre--'): return inner
            if node.op == '*': return inner.base if inner.kind == TypeKind.PTR else T_INT
            if node.op == '&': return ptr_to(inner)
            if node.op == '!': return T_INT
            return inner
        if isinstance(node, Assign): return self._type_of(node.target)
        if isinstance(node, Call):
            if node.ty and node.ty != T_INT: return node.ty
            if isinstance(node.func, Var):
                sym = self.scope.lookup(node.func.name)
                if sym and sym.ty.kind == TypeKind.FUNC: return sym.ty.ret
                if node.func.name in self.func_sigs: return self.func_sigs[node.func.name][0]
            return T_INT
        if isinstance(node, Index):
            if node.ty and node.ty != T_INT: return node.ty
            aty = self._type_of(node.arr)
            if aty.kind == TypeKind.PTR: return aty.base or T_INT
            if aty.kind == TypeKind.ARRAY: return aty.base or T_INT
            return T_INT
        if isinstance(node, Member):
            if node.ty and node.ty != T_INT: return node.ty
            ety = self._type_of(node.expr)
            sty = ety.base if (node.is_ptr and ety.kind == TypeKind.PTR) else ety
            if sty and sty.kind == TypeKind.STRUCT and sty.members:
                for mname, mty, _ in sty.members:
                    if mname == node.field_name: return mty
            return T_INT
        if isinstance(node, Ternary): return self._type_of(node.then)
        if isinstance(node, Comma): return self._type_of(node.exprs[-1]) if node.exprs else T_INT
        return T_INT
    ...
    def _gen_local_var(self, node: VarDecl):
        ty = node.ty
        # Infer array size from initializer e.g. long arr[] = {1,2,3}
        if ty.kind == TypeKind.ARRAY and ty.array_size == 0 and node.init:
            if isinstance(node.init, StrLit):
                ty = arr_of(ty.base, len(node.init.val) + 1)
            elif isinstance(node.init, BinOp) and node.init.op == '{}':
                cnt = node.init.left.val if isinstance(node.init.left, IntLit) else 0
                if cnt: ty = arr_of(ty.base, cnt)
            node.ty = ty
        sz = ty.size()
        if sz == 0: sz = 8
        al = ty.align()
