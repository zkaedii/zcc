...
static RegID get_or_create_var(LowerCtx *ctx, const char *name) {
    if (!name || (uintptr_t)name < 4096 || name[0] == '\0') return 0;
...
void node_name(struct Node *n, char *buf, unsigned len) { (void)n; (void)buf; (void)len; }
 ...
 */
 Function *zcc_ast_to_ir(ZCCNode *body_ast, const char *func_name)
 {
     ...
     /* Root cause fix: parameters must be hoisted in the exact order they are declared.
     * The func_name strings are now of the format: "func_name,param1,param2,..." */
     if (func_name) {
         const char *p = strchr(func_name, ',');
         if (p) {
             p++; /* skip comma */
             while (*p) {
                 ...
                 p_name[i] = '\0';
                 if (*p == ',') p++;
                 if (p_name[0]) (void)get_or_create_var(&ctx, p_name);
             }
         } else if (is_main_func(func_name)) {
...
static void ir_asm_lower_insn(IRAsmCtx *ctx, const Instr *ins, BlockID cur_block)
{
    ...
    switch (ins->op) {
        ...
        case OP_CALL: {
            /* System V AMD64 ABI: args in rdi, rsi, rdx, rcx, r8, r9; extras on stack */
            static const char *arg_regs[] = {"rdi","rsi","rdx","rcx","r8","r9"};
            int na = (int)ins->n_call_args;
            int stack_args = (na > 6) ? na - 6 : 0;
            int n_csave = 0;
            { int m = ctx->used_callee_saved_mask; while (m) { n_csave += m & 1; m >>= 1; } }
            /* Save caller-saved phys regs r10(idx 1) and r11(idx 2) that linear scan
               may have assigned to live vregs. 2 pushes = 16 bytes (alignment-neutral). */
            fprintf(f, "    pushq %%r10\n");
            fprintf(f, "    pushq %%r11\n");
            /* After prologue + n_csave pushes + 2 (r10/r11 save): RSP alignment depends on
               parity of (n_csave + 2) = parity of n_csave. For callq, RSP must be 16-aligned.
               Total additional pushes before callq: stack_args + pad.
               Need (n_csave + 2 + stack_args + pad) even → (n_csave + stack_args + pad) even. */
            int need_pad = (n_csave + stack_args) & 1;
            if (need_pad)
            ...
            /* Load register args 0..min(na,6)-1 into arg registers.
               Safe: phys pool is {rbx,r10,r11,r12-r15} — no overlap with arg regs.
               r10/r11 already saved above, so reading from their stack homes is fine. */
            { int rc = (na < 6) ? na : 6;
              for (int i = 0; i < rc; i++) {
                  ir_asm_load_to_rax(ctx, ins->call_args[i]);
            ...
            fprintf(f, "    xorl %%eax, %%eax\n");          /* al = 0 vector args (variadic) */
            fprintf(f, "    callq %s\n", ins->call_name);
            /* Clean up stack args + alignment pad */
            { int cleanup = stack_args * 8 + (need_pad ? 8 : 0);
              if (cleanup > 0) fprintf(f, "    addq $%d, %%rsp\n", cleanup);
            }
            /* Restore caller-saved phys regs (pop in reverse push order) */
            fprintf(f, "    popq %%r11\n");
            fprintf(f, "    popq %%r10\n");
            /* Store return value (in rax from callee) */
...
static void ir_asm_assign_alloca_offsets(IRAsmCtx *ctx)
{
    ...
    for (uint32_t bi = 0; bi < fn->n_blocks; bi++) {
        ...
        for (Instr *ins = blk->head; ins; ins = ins->next) {
            ...
            int off = (n < np) ? (-8 * (n + 1)) : (-8 * (int)(fn->n_regs + 2 + (n - np)));
...
static void ir_asm_emit_one_block(IRAsmCtx *ctx, BlockID bid)
{
    ...
    if (!ctx->callee_saved_emitted && ctx->used_callee_saved_mask) {
