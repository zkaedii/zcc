 ...
 */
ZCCNode *zcc_node_from(void *node) {
...
/* ── CG-IR-015 build fix: PassResult forward declaration ────────────────────
 * PassResult is defined in zcc_ir_opt_passes.h (included ~120 lines below)
 * but first used here in zcc_ir_bridge_dump_and_free.  Forward-declare it
 * so the TU compiles top-to-bottom without moving the #include.
 * Guards prevent double-definition when zcc_ir_opt_passes.h is the real file
 * and already defines PassResult via its own #ifndef guard.               */
#ifndef ZCC_BLOCK_NAME_LEN
#  define ZCC_BLOCK_NAME_LEN 64
#endif
#ifndef ZCC_PR_MAX_BLOCKS
#  define ZCC_PR_MAX_BLOCKS  8192
#endif
#ifndef PASS_RESULT_TYPEDEF_DONE
#  define PASS_RESULT_TYPEDEF_DONE
typedef struct {
    uint32_t order[ZCC_PR_MAX_BLOCKS];
    uint32_t n_blocks;
    char     block_names[ZCC_PR_MAX_BLOCKS][ZCC_BLOCK_NAME_LEN];
    uint32_t licm_hoisted;
    uint32_t dce_instrs_removed;
} PassResult;
#endif
...
static void ir_asm_lower_insn(IRAsmCtx *ctx, const Instr *ins,
  ...
  case OP_CALL: {
    ...
    /* Caller-saved phys regs that linear scan might have assigned: r10(1),
       r11(2). Save them before the call destroys them. 2 pushes = 16 bytes,
       alignment-neutral. */
    fprintf(f, "    pushq %%r10\n");
    fprintf(f, "    pushq %%r11\n");
    /* Alignment: after prologue + n_csave + 2 (r10/r11), need RSP 16-aligned at
       callq. Total extra pushes = stack_args + pad. (n_csave + 2 + stack_args +
       pad) must be even. */
    int need_pad = (n_csave + stack_args) &
                   1; /* 2 is even, so parity = n_csave + stack_args */
    if (need_pad)
...
static void ir_asm_assign_alloca_offsets(IRAsmCtx *ctx) {
  ...
  for (uint32_t bi = 0; bi < fn->n_blocks; bi++) {
    ...
    for (Instr *ins = blk->head; ins; ins = ins->next) {
      ...
      int off = (n < np)
                    ? (-8 * (n + 1))
                    : (ctx->slot_base - 8 * (int)(fn->n_regs + 2 + (n - np)));
      if (ins->dst < MAX_INSTRS) {
        ctx->alloca_off[ins->dst] = off;
        fprintf(stderr, "[ALLOCA] dst=%u off=%d\n", ins->dst, off);
      }
      n++;
    }
  }
...
int zcc_run_passes_emit_body_pgo(ZCCNode *body_ast, const char *profile_path,
   ...
   * CG-IR-016: ir_extra hoisted out of block scope so save/restore below
   * can use it to address the bottom of the frame without colliding with
   * IR spill slots that grow down from slot_base.                        */
  int ir_extra = 0;
  {
    ...
    for (uint32_t bi = 0; bi < fn->n_blocks; bi++) {
      ...
      for (Instr *ins = b->head; ins; ins = ins->next) {
        if (ins->dst >= max_reg)
          max_reg = ins->dst + 1;
        for (uint32_t s = 0; s < ins->n_src && s < MAX_OPERANDS; s++)
          if (ins->src[s] >= max_reg)
            max_reg = ins->src[s] + 1;
        if (ins->op == OP_ALLOCA) {
          n_alloca++;
          if (ins->imm > 0)
            alloca_bytes += ins->imm;
        }
      }
    }
    ir_extra = 8 * ((int)max_reg + n_alloca + 8) + (int)alloca_bytes;
    ir_extra = (ir_extra + 15) & ~15;
    fprintf(out, "    subq $%d, %%rsp\n", ir_extra);
  }

  /* CG-IR-016: save callee-saved regs at the BOTTOM of ir_extra.
   *
   * Bug CG-IR-016: %rbx (and r12-r15) are used 5711× in the IR body but
   * body_only=1 suppresses the push/pop that the non-body path emits.
   * ir_asm_lower_insn with body_only skips callee-saved push, so those regs
   * were silently clobbered across calls into the IR body.
   *
   * Fix: use movq into fixed offsets at the BOTTOM of the ir_extra frame
   * (not slot_base-N, which collides with spills growing downward).
   * Layout: -(stack_size + ir_extra - 8..40) = last 40 bytes of ir_extra.
   * The +8 headroom in ir_extra = 8*(max_reg + n_alloca + 8) guarantees
   * at least 64 bytes of padding at the bottom — 40 consumed by 5 regs.  */
  fprintf(out, "    movq %%rbx, %d(%%rbp)\n", -(stack_size + ir_extra - 8));
  fprintf(out, "    movq %%r12, %d(%%rbp)\n", -(stack_size + ir_extra - 16));
  fprintf(out, "    movq %%r13, %d(%%rbp)\n", -(stack_size + ir_extra - 24));
  fprintf(out, "    movq %%r14, %d(%%rbp)\n", -(stack_size + ir_extra - 32));
  fprintf(out, "    movq %%r15, %d(%%rbp)\n", -(stack_size + ir_extra - 40));

  ir_asm_emit_function_body(&ctx);

  /* CG-IR-016: restore callee-saved regs from same fixed slots */
