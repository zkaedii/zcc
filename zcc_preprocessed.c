 ...
 *           static/extern/const qualifiers, 
...
struct Node  *node_default_case(struct Node *n);   /* ND_SWITCH: default case node or ((void*)0) */
...
struct ZCCNode {
    ...
    ZCCNode  *default_body;   /* ((void*)0) if no default */
    ZCCNode **stmts;
...
/* Allocate and zero a single ir_node_t.  Never returns ((void*)0) (exits on OOM). */
ir_node_t   *ir_node_alloc(void);
...
/* Return cutoff so stage2 can't miscompile  constant (GDB showed
 * node=0x10000). */
    ...
    if (zcc_run_passes_emit_body_pgo && zcc_node_from) {
      ...
      if (ir_ast) {
        ...
        ir_ok = zcc_run_passes_emit_body_pgo(
            ir_ast, ((void*)0), func->func_def_name, cc->out, stack_size,
  ...
  if (node->kind == ND_IF) {
    if (node->cond && node->cond->kind == ND_NUM) {
      if (node->cond->int_val == 0) {
        /* condition is 0: replace with else body or empty block */
        if (node->else_body) {
        ...
        /* condition is 1: replace with then body */
        if (node->then_body) {
