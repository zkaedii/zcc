...
static int is_type_token(Compiler *cc) {
  if (cc->tk >= TK_INT && cc->tk <= TK_DOUBLE)
    return 1;
  if (cc->tk >= TK_STATIC && cc->tk <= TK_INLINE)
    return 1;
  if (cc->tk == TK_STRUCT)
    return 1;
  if (cc->tk == TK_UNION)
...
static Type *find_struct(Compiler *cc, char *tag) {
  ...
  for (i = 0; i < cc->num_structs; i++) {
    if (strcmp(cc->structs[i]->tag, tag) == 0)
      return cc->structs[i];
  }
  return 0;
}
...
static long long parse_const_expr(Compiler *cc) {
  return parse_const_expr_ternary(cc);
}
...
static long long parse_const_expr_lor(Compiler *cc) {
  ...
  while (cc->tk == TK_LOR) {
    next_token(cc);
    long long rhs = parse_const_expr_land(cc);
    val = val || rhs;
  }
  return val;
}
static long long parse_const_expr_land(Compiler *cc) {
  ...
  while (cc->tk == TK_LAND) {
    next_token(cc);
    long long rhs = parse_const_expr_bor(cc);
    val = val && rhs;
  }
  return val;
}
...
static long long parse_const_expr_eq(Compiler *cc) {
  ...
  while (cc->tk == TK_EQ || cc->tk == TK_NE) {
    ...
    next_token(cc);
    if (tk == TK_EQ)
      val = (val == parse_const_expr_rel(cc));
    else
      val = (val != parse_const_expr_rel(cc));
  }
  return val;
...
static long long parse_const_expr_rel(Compiler *cc) {
  ...
  while (cc->tk == TK_LT || cc->tk == TK_GT || cc->tk == TK_LE ||
         cc->tk == TK_GE) {
    int tk = cc->tk;
    next_token(cc);
    if (tk == TK_LT)
      val = (val < parse_const_expr_shift(cc));
    else if (tk == TK_GT)
      val = (val > parse_const_expr_shift(cc));
    else if (tk == TK_LE)
      val = (val <= parse_const_expr_shift(cc));
...
static Type *parse_struct_or_union(Compiler *cc, int is_union) {
...

  {
    ...
    int current_bitfield_type_size;
    ...
    while (cc->tk != TK_RBRACE) {
      ...
      if (cc->tk == TK_COLON) {
        ...
        next_token(cc);
        if (cc->tk == TK_NUM) {
          bitfield_width = (int)cc->tk_val;
          next_token(cc);
        } else {
          bitfield_width = (int)parse_const_expr(cc);
        }
      }

      if (fname[0] == '\0' &&
          (ftype->kind == TY_STRUCT || ftype->kind == TY_UNION) &&
          ftype->tag[0] == '\0') {
        StructField *anon_field = ftype->fields;
        ...
        if (is_union) {
          base_offset = 0;
          if (type_size(ftype) > max_size)
            max_size = type_size(ftype);
        } else {
          if (current_bit_offset > 0) {
            offset += current_bitfield_type_size;
            current_bit_offset = 0;
            current_bitfield_type_size = 0;
          }
          if (falign > 1) {
        ...
        while (anon_field) {
          StructField *new_field =
              (StructField *)cc_alloc(cc, sizeof(StructField));
          strncpy(new_field->name, anon_field->name, MAX_IDENT - 1);
          new_field->type = anon_field->type;
          new_field->bitfield_width = anon_field->bitfield_width;
          new_field->bitfield_offset = anon_field->bitfield_offset;
          new_field->offset = base_offset + anon_field->offset;
          new_field->next = 0;

          if (last_field) {
            last_field->next = new_field;
          } else {
            stype->fields = new_field;
          }
        ...
        if (cc->tk == TK_SEMI) {
          ...
          error(cc, "expected ';' after anonymous struct/union");
          /* Force progress to prevent infinite block loops */
          if (cc->tk == TK_RBRACE || cc->tk == TK_EOF)
            break;
          next_token(cc);
        }
        continue;
      ...
      if (is_union) {
        ...
        if (has_colon) {
          if (bitfield_width == 0 ||
              type_size(ftype) != current_bitfield_type_size ||
              (current_bit_offset + bitfield_width > type_size(ftype) * 8)) {
            if (current_bit_offset > 0) {
              offset += current_bitfield_type_size;
            }
            if (falign > 1) {
            ...
            current_bit_offset = 0;
            current_bitfield_type_size = type_size(ftype);
          }
          field->offset = offset;
          field->bitfield_offset = current_bit_offset;
          current_bit_offset += bitfield_width;
        } else {
          if (current_bit_offset > 0) {
            offset += current_bitfield_type_size;
            current_bit_offset = 0;
      ...
      while (cc->tk == TK_COMMA) {
        ...
        int bitfield_width2 = 0;
        int has_colon2 = 0;
        if (cc->tk == TK_COLON) {
          has_colon2 = 1;
          next_token(cc);
          if (cc->tk == TK_NUM) {
            bitfield_width2 = (int)cc->tk_val;
            next_token(cc);
          } else {
            bitfield_width2 = (int)parse_const_expr(cc);
          }
        }
        ...
        field2->type = ftype2;
        field2->bitfield_width = bitfield_width2;
        field2->bitfield_offset = 0;
        falign2 = type_align(ftype2);
        if (falign2 > max_align)
          max_align = falign2;
        if (is_union) {
          field2->offset = 0;
          if (type_size(ftype2) > max_size)
            max_size = type_size(ftype2);
        } else {
          if (has_colon2) {
            if (bitfield_width2 == 0 ||
                type_size(ftype2) != current_bitfield_type_size ||
                (current_bit_offset + bitfield_width2 >
                 type_size(ftype2) * 8)) {
              if (current_bit_offset > 0) {
                offset += current_bitfield_type_size;
              }
              ...
              current_bit_offset = 0;
              current_bitfield_type_size = type_size(ftype2);
            }
            field2->offset = offset;
            field2->bitfield_offset = current_bit_offset;
            current_bit_offset += bitfield_width2;
          } else {
            if (current_bit_offset > 0) {
              offset += current_bitfield_type_size;
  ...
  expect(cc, TK_RBRACE);
  if (stype->tag[0] && (strcmp(stype->tag, "yyStackEntry") == 0 ||
                        strcmp(stype->tag, "yyParser") == 0 ||
                        strcmp(stype->tag, "Walker") == 0)) {
    int n = 0;
    ...
    printf("DEBUG: Parsed %s with %d fields (Type=%p, fields=%p)\n", stype->tag,
           n, (void *)stype, (void *)stype->fields);
    fflush(stdout);
  }
  return stype;
}
...
Type *parse_type(Compiler *cc) {
  ...
  for (;;) {
    if (cc->tk == TK_STATIC) {
      is_static = 1;
      next_token(cc);
    } else if (cc->tk == TK_EXTERN) {
      is_extern = 1;
      next_token(cc);
    } else if (cc->tk == TK_CONST) {
      next_token(cc);
    } else if (cc->tk == TK_VOLATILE) {
      next_token(cc);
    } else if (cc->tk == TK_INLINE) {
      next_token(cc);
    } else if (cc->tk == TK_AUTO) {
      next_token(cc);
    } else if (cc->tk == TK_REGISTER) {
      next_token(cc);
    } else if (cc->tk == TK_TYPEDEF) {
      is_typedef_kw = 1;
      next_token(cc);
    } else if (cc->tk == TK_UNSIGNED) {
      is_unsigned = 1;
      next_token(cc);
    } else if (cc->tk == TK_SIGNED) {
      is_signed = 1;
      next_token(cc);
      ...
      is_char = 1;
      next_token(cc);
    } else if (cc->tk == TK_DOUBLE) {
      is_double = 1;
      next_token(cc);
    } else if (cc->tk == TK_FLOAT) {
      is_float = 1;
      next_token(cc);
    } else if (cc->tk == TK_VOID) {
      is_void = 1;
      next_token(cc);
    } else
      break;
      ...
      if (cc->tk == TK_LBRACKET) {
        ...
        while (cc->tk == TK_LBRACKET) {
          ...
          if (cc->tk != TK_RBRACKET)
            len = (int)parse_const_expr(cc);
          expect(cc, TK_RBRACKET);
          if (arr_num < 16)
            arr_lens[arr_num++] = len;
        }
        while (arr_num > 0) {
          arr_num--;
        ...
        if (cc->tk != TK_RPAREN) {
          ...
          parse_params_inner:
            while (cc->tk != TK_RPAREN) {
              ...
              if (ptype->kind == TY_ARRAY)
                ptype = type_ptr(cc, ptype->base);
...
Node *parse_primary(Compiler *cc) {
  ...
  if (cc->tk == TK_IDENT) {
    ...
    if (strcmp(cc->tk_text, "yypParser") == 0 ||
        strcmp(cc->tk_text, "pParser") == 0) {
      printf("DEBUG: parse_primary lookup '%s'. sym=%p, sym->type=%p, base=%p, "
             "base_tag='%s'\n",
             cc->tk_text, (void *)sym, sym ? (void *)sym->type : NULL,
             (sym && sym->type) ? (void *)sym->type->base : NULL,
             (sym && sym->type && sym->type->base && sym->type->base->tag[0])
                 ? sym->type->base->tag
                 : "<none>");
      fflush(stdout);
    }
    ...
    if (sym) {
      ...
      if (strcmp(n->name, "stdin") == 0 || strcmp(n->name, "stdout") == 0 ||
          strcmp(n->name, "stderr") == 0) {
        n->type = type_ptr(cc, cc->ty_char);
      } else {
        n->type = cc->ty_int;
      }
  ...
  if (cc->tk == TK_SIZEOF) {
    ...
    if (cc->tk == TK_LPAREN) {
      ...
      is_type_in_parens = 0;
      if (pk >= TK_INT) {
        if (pk <= TK_INLINE || pk == TK_STRUCT || pk == TK_UNION ||
            pk == TK_ENUM) {
          is_type_in_parens = 1;
        }
      }
  ...
  {
    char buf[512];
    sprintf(buf, "unexpected token %d ('%s') in expression", cc->tk,
            cc->tk_text);
    error(cc, buf);
    next_token(cc);
...
static StructField *find_struct_member(Type *type, const char *name,
                                       int *out_offset) {
  StructField *f;
  if (!type) {
    return 0;
  ...
  for (;;) {
    ...
    if (cc->tk == TK_DOT) {
      ...
      if (n->type) {
        ...
        if (f) {
          ...
          sprintf(
              buf,
              "unknown struct member '%s' in struct '%s' (type=%p, fields=%p)",
              cc->tk_text,
              (n->type && n->type->tag[0]) ? n->type->tag : "<anon>",
              (void *)n->type, n->type ? (void *)n->type->fields : NULL);
          error(cc, buf);
    ...
    if (cc->tk == TK_ARROW) {
      ...
      if (deref->type) {
        ...
        if (f) {
          ...
          char buf[256];
          sprintf(buf,
                  "unknown struct member '%s' after -> in struct '%s' "
                  "(type=%p, fields=%p)",
                  cc->tk_text,
                  (deref->type && deref->type->tag[0]) ? deref->type->tag
                                                       : "<anon>",
                  (void *)deref->type,
                  deref->type ? (void *)deref->type->fields : NULL);
          error(cc, buf);
          n->type = cc->ty_int;
        }
      } else {
    ...
    if (cc->tk == TK_LPAREN) {
      ...
      if (n->kind == ND_VAR) {
        if (!n->sym)
          is_direct = 1;
        else if (n->sym->type && n->sym->type->kind == TY_FUNC)
          is_direct = 1;
      }
      if (is_direct) {
        /* function call */
        ...
        while (cc->tk != TK_RPAREN) {
          ...
          if (call->num_args < MAX_CALL_ARGS) {
            ...
            if (sym && sym->type && sym->type->kind == TY_FUNC && call->num_args < sym->type->num_params) {
              Type *ptype = sym->type->params[call->num_args];
              if (arg->type && arg->type->kind != ptype->kind) {
                Node *cast = node_new(cc, ND_CAST, line);
                cast->lhs = arg;
                cast->type = ptype;
                cast->cast_type = ptype;
                arg = cast;
              }
            } else {
              if (arg->type && arg->type->kind == TY_FLOAT) {
                Node *cast = node_new(cc, ND_CAST, line);
                cast->lhs = arg;
                cast->type = cc->ty_double;
                cast->cast_type = cc->ty_double;
                arg = cast;
              }
            }
            call->args[call->num_args] = arg;
      ...
      if (n->kind == ND_MEMBER || n->kind == ND_DEREF || n->kind == ND_CAST ||
          (n->kind == ND_VAR && !is_direct)) {
        ...
        while (cc->tk != TK_RPAREN) {
          ...
          if (call->num_args < MAX_CALL_ARGS) {
            Node *arg = parse_assign(cc);
            if (func_type && call->num_args < func_type->num_params) {
              Type *ptype = func_type->params[call->num_args];
              if (arg->type && arg->type->kind != ptype->kind) {
                Node *cast = node_new(cc, ND_CAST, line);
                cast->lhs = arg;
                cast->type = ptype;
        ...
        if (n->type) {
          ...
          } else if (n->type->kind == TY_PTR && n->type->base &&
                     n->type->base->kind == TY_FUNC) {
...
Node *parse_unary(Compiler *cc) {
  ...
  if (cc->tk == TK_SIZEOF) {
    ...
    if (cc->tk == TK_LPAREN) {
      ...
      if ((pk >= TK_INT && pk <= TK_DOUBLE) ||
          (pk >= TK_STATIC && pk <= TK_INLINE) || pk == TK_STRUCT ||
          pk == TK_UNION || pk == TK_ENUM || pk == TK_TYPEDEF) {
        is_type = 1;
      } else if (pk == TK_IDENT) {
  ...
  if (cc->tk == TK_LPAREN) {
    int pk = peek_token(cc);
    int is_cast = 0;
    if (pk == TK_INT || pk == TK_CHAR || pk == TK_VOID || pk == TK_STRUCT ||
        pk == TK_UNION || pk == TK_ENUM || pk == TK_LONG || pk == TK_SHORT ||
        pk == TK_UNSIGNED || pk == TK_SIGNED || pk == TK_FLOAT ||
        pk == TK_DOUBLE) {
      is_cast = 1;
    } else if (pk == TK_IDENT) {
      Symbol *sym = scope_find(cc, cc->peek_text);
...
static Type *promote_type(Compiler *cc, Type *t1, Type *t2) {
  ...
  if (type_size(t1) >= 8 || type_size(t2) >= 8) {
    if (is_unsigned_type(t1) || is_unsigned_type(t2))
      return cc->ty_ulong;
    return cc->ty_long;
  }
  if (is_unsigned_type(t1) || is_unsigned_type(t2))
      ...
      for (;;) {
        ...
        if (vname[0]) {
          if (is_typedef) {
            ...
            sprintf(mangled, "%s__%s__%d", cc->current_func, vname,
                    cc->label_count++);
            ...
            if (cc->tk == TK_ASSIGN) {
              ...
              if (cc->tk == TK_LBRACE) {
                ...
                while (depth > 0 && cc->tk != TK_EOF) {
                  if (depth == 1 && cc->tk != TK_RBRACE && cc->tk != TK_COMMA &&
                      last_comma) {
                    top_elems++;
                    last_comma = 0;
                  }
                  ...
                  while (depth > 0 && cc->tk != TK_EOF) {
                    ...
                    if (cc->tk == TK_LBRACE) {
                      ...
                      if (cc->pos == prev_pos) {
                        ...
                        if (no_progress_count > 100) {
                          error(cc,
                                "array parser stuck (possible infinite loop)");
                          break;
                        }
                      } else {
                        no_progress_count = 0;
                      ...
                      if (init_count >= init_cap) {
                        ...
                        init_cap *= 2;
                        inits =
                            (Node **)cc_alloc(cc, sizeof(Node *) * init_cap);
                  ...
                  /* Now emit field assignments */
                  {
                    ...
                    while (sf && fi < init_count_s) {
                      ...
                      if (cnt >= cap) {
                        ...
                        int ji;
                        new_stmts =
                            (Node **)cc_alloc(cc, sizeof(Node *) * cap * 2);
                        for (ji = 0; ji < cnt; ji++)
                          new_stmts[ji] = block->stmts[ji];
                        block->stmts = new_stmts;
                        cap = cap * 2;
                      }
...
static Node *parse_func_def(Compiler *cc, Type *ret_type, char *name,
                            int is_static) {
  Node *func;
  int line;
  int i;
  ...
  func->param_names_buf = (char (*)[128])cc_alloc(cc, 128 * MAX_PARAMS);
  func->param_types = (Type **)cc_alloc(cc, 8 * MAX_PARAMS);
    ...
    parse_fparams:
      while (cc->tk != TK_RPAREN) {
        ...
        if (func->num_params < MAX_PARAMS) {
          ...
          strncpy(func->param_names_buf[func->num_params], pname,
                  MAX_IDENT - 1);
        ...
        if (cc->tk == TK_COMMA) {
          ...
          /* Unexpected token (e.g. ->) after parameter — skip to ) to avoid
           * cascade */
          if (cc->tk == TK_ARROW) {
            error(cc,
                  "unexpected '->' in parameter list (missing ',' or ')'?)");
          } else {
...
Node *parse_program(Compiler *cc) {
  ...
  while (cc->tk != TK_EOF) {
       ...
       so we can detect function defs before params are consumed */
    {
      ...
      /* get name — also handle grouped declarator (*name)(params) for function
       * pointers */
      name[0] = 0;
      if (cc->tk == TK_LPAREN) {
        int gpk;
        ...
        if (gpk == TK_STAR) {
          ...
          while (cc->tk == TK_LBRACKET) {
            ...
            next_token(cc);
            if (cc->tk != TK_RBRACKET)
              alen = (int)parse_const_expr(cc);
            expect(cc, TK_RBRACKET);
            if (arr_num < 8)
              arr_lens[arr_num++] = alen;
          }
          expect(cc, TK_RPAREN);
    ...
    after_name:
      ...
      if (cc->tk == TK_LPAREN) {
        if (!is_typedef_kw) {
          ...
          /* only link into list if definition (has body); forward decl has body
           * == 0 */
          if (func->body) {
            func->next = 0;
            if (!head) {
...
static void ZCC_IR_INIT(void) {
  if (getenv("ZCC_EMIT_IR"))
    g_emit_ir = 1;
  /* ZCC_IR_BACKEND uses compiler_passes.c bridge path directly via
   * getenv("ZCC_IR_BACKEND") in codegen_func — it does NOT need
   * g_emit_ir or g_ir_module. Setting g_emit_ir=1 here caused
   * ir_node_t accumulation of 1.44GB+ in g_ir_module (OOM). */
  if (g_emit_ir && !getenv("ZCC_IR_BACKEND"))
    g_ir_module = ir_module_create();
}

static void ZCC_IR_FUNC_BEGIN(const char *fname, ir_type_t ret_ty,
                              int num_params) {
  if (g_emit_ir) {
...
/* Drain after each function in ZCC_IR_BACKEND mode to prevent OOM.
 * In backend mode, ir_node_t IR is consumed by compiler_passes.c and
 * never read from g_ir_module, so we free + recreate after each function.
 * Without this: ~2000 funcs x 2000 nodes x 360B/node = 1.44GB OOM. */
static void ZCC_IR_FUNC_DRAIN(void) {
  if (g_emit_ir && g_ir_module && getenv("ZCC_IR_BACKEND")) {
    ir_module_free(g_ir_module);
    g_ir_module = ir_module_create();
    g_ir_cur_func = 0;
  }
...
static void ZCC_EMIT_BINARY(ir_op_t op, ir_type_t ty, const char *dst,
                            const char *s1, const char *s2, int line) {
  if (g_emit_ir && g_ir_cur_func) {
    ir_emit(g_ir_cur_func, op, ty, dst, s1, s2, 0, 0, line);
  }
}

static void ZCC_EMIT_UNARY(ir_op_t op, ir_type_t ty, const char *dst,
...
static void ZCC_EMIT_CONST(ir_type_t ty, const char *dst, long immval,
                           int line) {
  if (g_emit_ir && g_ir_cur_func) {
...
static void ZCC_EMIT_STORE(ir_type_t ty, const char *addr, const char *val,
                           int line) {
  if (g_emit_ir && g_ir_cur_func) {
...
static void ZCC_EMIT_LOAD(ir_type_t ty, const char *dst, const char *addr,
                          int line) {
  if (g_emit_ir && g_ir_cur_func) {
...
static void ZCC_EMIT_CALL(ir_type_t ty, const char *dst, const char *fname,
                          int line) {
  if (g_emit_ir && g_ir_cur_func) {
...
static void ZCC_EMIT_FBINARY(ir_op_t op, const char *dst, const char *s1,
...
static ir_type_t ir_map_type(Type *ty) {
  if (!ty)
    return IR_TY_I64; /* default to 64-bit signed */
...
static void ir_bridge_func_end(void) {
  ZCC_IR_FUNC_END();
  /* NOTE: no drain here — drain caused infinite loops in self-compiled zcc2.
   * Accumulation is prevented by setting g_emit_ir=0 in ZCC_IR_BACKEND mode
   * (see ir_emit_dispatch.h ZCC_IR_INIT fix). */
}
...
static void ir_emit_var_load(Node *node) {
  ...
  if (node->type &&
      (node->type->kind == TY_ARRAY || node->type->kind == TY_STRUCT ||
...
void codegen_expr(Compiler *cc, Node *node) {
  ...
  case ND_ASSIGN: {
    ...
    if (node->lhs && node->lhs->kind == ND_VAR && node->lhs->sym &&
      ...
      if (node->lhs->type && !node_type_unsigned(node->lhs) &&
          !is_pointer(node->lhs->type)) {
        if (node->lhs->type->size == 4) {
          if (!backend_ops)
            fprintf(cc->out, "    cltq\n");
        } else if (node->lhs->type->size == 1)
          fprintf(cc->out, "    movsbq %%al, %%rax\n");
    ...
    if (node->lhs->kind == ND_MEMBER && node->lhs->member_size > 0) {
      pop_reg(cc, "r11");
      if ((node->lhs->type && (node->lhs->type->kind == TY_STRUCT ||
                               node->lhs->type->kind == TY_UNION)) ||
          node->lhs->member_size > 8) {
        fprintf(cc->out, "    pushq %%rsi\n");
        fprintf(cc->out, "    pushq %%rdi\n");
        fprintf(cc->out, "    pushq %%rcx\n");
        fprintf(cc->out, "    movq %%r11, %%rsi\n");
        ...
        /* If member type is pointer/func, always use movq regardless of
         * member_size */
  ...
  case ND_COMPOUND_ASSIGN:
    ...
    if (node->lhs->kind == ND_VAR && node->lhs->sym &&
      ...
      case ND_SUB:
        ...
        if (backend_ops)
          backend_ops->emit_binary_op(cc, ND_SUB);
        else
          fprintf(cc->out, "    subq %%r11, %%rax\n");
        break;
      case ND_MUL:
        if (backend_ops)
          backend_ops->emit_binary_op(cc, ND_MUL);
        else
      ...
      case ND_BAND:
        if (backend_ops)
          backend_ops->emit_binary_op(cc, ND_BAND);
        else
          fprintf(cc->out, "    andq %%r11, %%rax\n");
        break;
      case ND_BOR:
        if (backend_ops)
          backend_ops->emit_binary_op(cc, ND_BOR);
        else
          fprintf(cc->out, "    orq %%r11, %%rax\n");
        break;
      case ND_BXOR:
        if (backend_ops)
          backend_ops->emit_binary_op(cc, ND_BXOR);
        else
          fprintf(cc->out, "    xorq %%r11, %%rax\n");
        break;
      case ND_SHL:
        if (backend_ops)
          backend_ops->emit_binary_op(cc, ND_SHL);
        else
      ...
      case ND_SHR:
        if (node->lhs->type && is_unsigned_type(node->lhs->type))
          if (backend_ops)
            backend_ops->emit_binary_op(cc, ND_SHR);
          else
            fprintf(cc->out, "    movq %%r11, %%rcx\n    shrq %%cl, %%rax\n");
        else if (backend_ops)
          backend_ops->emit_binary_op(cc, ND_SHR);
    ...
    {
      char *dst = ir_bridge_fresh_tmp();
      ir_emit_binary_op(node->compound_op, node->lhs->type, "ca_lhs", "ca_rhs",
                        node->line);
    }
    return;
    ...
    if (node->lhs->type && is_pointer(node->lhs->type)) {
      if (node->rhs->type && is_pointer(node->rhs->type)) {
          ...
          fprintf(cc->out, "    subq %%r11, %%rax\n");
        {
          ...
          esz = ptr_elem_size(node->lhs->type);
          if (esz > 1) {
            if (backend_ops) { /* no-op for hello.c */
            } else {
  ...
  case ND_NEG: {
    ...
    if (node->type && is_float_type(node->type)) {
      ...
      {
        char *dst = ir_bridge_fresh_tmp();
        ZCC_EMIT_UNARY(IR_NEG, ir_map_type(node->type), dst, "f_lhs",
                       node->line);
      }
      return;
    }
  ...
  case ND_GE: {
    ...
    use32 = node->lhs && node->lhs->type && node->rhs && node->rhs->type &&
            type_size(node->lhs->type) == 4 && type_size(node->rhs->type) == 4;
    codegen_expr_checked(cc, node->lhs);
    ir_save_result(lhs_ir);
    push_reg(cc, "rax");
    codegen_expr_checked(cc, node->rhs);
    ...
    if (backend_ops) {
      ...
      if (uns) {
        ...
        case ND_NE:
          ...
          break;
        case ND_LT:
          fprintf(cc->out, "    blo .L%d\n", lbl_true);
          break;
        case ND_LE:
          fprintf(cc->out, "    bls .L%d\n", lbl_true);
          break;
        case ND_GT:
          fprintf(cc->out, "    bhi .L%d\n", lbl_true);
          break;
        case ND_GE:
          fprintf(cc->out, "    bhs .L%d\n", lbl_true);
          break;
        }
      } else {
        switch (node->kind) {
        ...
        case ND_NE:
          ...
          break;
        case ND_LT:
          fprintf(cc->out, "    blt .L%d\n", lbl_true);
          break;
        case ND_LE:
          fprintf(cc->out, "    ble .L%d\n", lbl_true);
          break;
        case ND_GT:
          fprintf(cc->out, "    bgt .L%d\n", lbl_true);
          break;
        case ND_GE:
          fprintf(cc->out, "    bge .L%d\n", lbl_true);
          break;
        }
      }
  ...
  case ND_VA_ARG: {
    ...
    /* Increment overflow_arg_area by 8 (even for floats, stack passing is
     * 8-aligned) */
  ...
  case ND_CAST: {
    ...
    if (node->cast_type) {
      int src_size =
          node->lhs && node->lhs->type ? type_size(node->lhs->type) : 4;
      ...
      case 2:
        ...
        break;
      case 4:
        if (node->cast_type && !is_float_type(node->cast_type) && node->lhs &&
            node->lhs->type && is_float_type(node->lhs->type)) {
          fprintf(cc->out, "    movq %%rax, %%xmm0\n");
          fprintf(cc->out, "    cvttsd2si %%xmm0, %%rax\n");
        } else if (node->cast_type->kind == TY_UINT ||
                   node->cast_type->kind == TY_ULONG) {
          if (!backend_ops)
            fprintf(cc->out, "    movl %%eax, %%eax\n");
      ...
      case 8:
        if (node->cast_type && is_float_type(node->cast_type) && node->lhs &&
          ...
          fprintf(cc->out, "    movq %%xmm0, %%rax\n");
        } else if (node->cast_type && !is_float_type(node->cast_type) &&
                   node->lhs && node->lhs->type &&
                   is_float_type(node->lhs->type)) {
          fprintf(cc->out, "    movq %%rax, %%xmm0\n");
          fprintf(cc->out, "    cvttsd2si %%xmm0, %%rax\n");
        } else if (src_size == 4 &&
                   !is_pointer(node->lhs ? node->lhs->type : 0)) {
  ...
  case ND_PRE_INC:
    if (node->lhs && node->lhs->kind == ND_VAR && node->lhs->sym &&
      ...
      if (backend_ops) {
        fprintf(cc->out, "    ldr r3, =%d\n", esz);
        if (strcmp(reg, "rax") == 0)
          fprintf(cc->out, "    adds r0, r0, r3\n");
        else
          fprintf(cc->out, "    adds r1, r1, r3\n");
      } else
    ...
    {
      char *dst = ir_bridge_fresh_tmp();
      ZCC_EMIT_UNARY(IR_CAST, ir_map_type(node->lhs->type), dst, "pre_inc",
                     node->line);
    }
    return;
  ...
  case ND_PRE_DEC:
    if (node->lhs && node->lhs->kind == ND_VAR && node->lhs->sym &&
      ...
      if (backend_ops) {
        fprintf(cc->out, "    ldr r3, =%d\n", esz);
        if (strcmp(reg, "rax") == 0)
          fprintf(cc->out, "    subs r0, r0, r3\n");
        else
          fprintf(cc->out, "    subs r1, r1, r3\n");
      } else
    ...
    {
      char *dst = ir_bridge_fresh_tmp();
      ZCC_EMIT_UNARY(IR_CAST, ir_map_type(node->lhs->type), dst, "pre_dec",
                     node->line);
    }
    return;
  ...
  case ND_POST_INC:
    ...
    {
      char *dst = ir_bridge_fresh_tmp();
      ZCC_EMIT_UNARY(IR_CAST, ir_map_type(node->lhs->type), dst, "post_inc",
                     node->line);
    }
    return;
  ...
  case ND_POST_DEC:
    ...
    {
      char *dst = ir_bridge_fresh_tmp();
      ZCC_EMIT_UNARY(IR_CAST, ir_map_type(node->lhs->type), dst, "post_dec",
                     node->line);
    }
    return;
  ...
  case ND_CALL: {
    ...
    if (!node->func_name[0] && !node->lhs) {
      error_at(cc, node->line,
               "codegen_expr: ND_CALL no func_name and no callee");
      fprintf(cc->out, "    movq $0, %%rax\n");
      return;
    }
    if (!node->args) {
    ...
    /* System V AMD64: no shadow space required. Space for 7th+ gp args and 9th+
     * fp args is left on the stack. */
    {
      int temp_gp = 0, temp_fp = 0;
      for (i = 0; i < nargs; i++) {
        if (node->args[i] && node->args[i]->type &&
            is_float_type(node->args[i]->type)) {
          if (temp_fp < 8)
            temp_fp++;
        } else {
          if (temp_gp < 6)
    ...
    for (i = nargs - 1; i >= 0; i--) {
      ...
      ZCC_EMIT_ARG(ir_map_type(node->args[i] ? node->args[i]->type : 0),
                   &args_ir_1d[i * 32], node->line);
      push_reg(cc, "rax");
    }
    ...
    if (node->func_name[0] == 0 && node->lhs) {
      ...
      if (fsym && fsym->type && fsym->type->params) {
        for (int j = 0; j < fsym->type->num_params; j++) {
          if (is_float_type(fsym->type->params[j]))
            nparams_fp++;
          else
            nparams_gp++;
        }
...
extern void zcc_node_free(struct ZCCNode *z);
extern int zcc_run_passes_emit_body_pgo(struct ZCCNode *body,
                                        const char *profile, const char *name,
                                        void *out, int stack_size,
                                        int num_params, int end_label1,
                                        int end_label2);

#pragma weak zcc_run_passes_emit_body_pgo
#pragma weak zcc_node_from
#pragma weak zcc_node_free
...
void codegen_func(Compiler *cc, Node *func) {
  ...
  if (!backend_ops) {
    for (i = 0; i < func->num_params; i++) {
      ...
      if (func->param_types[i]->kind == TY_STRUCT ||
          func->param_types[i]->kind == TY_UNION) {
        if (i < 6) {
...
static long long eval_const_expr_p4(Node *elem, int *ok) {
  ...
  if (elem->kind == ND_ADD)
    return eval_const_expr_p4(elem->lhs, ok) +
           eval_const_expr_p4(elem->rhs, ok);
  if (elem->kind == ND_SUB)
    return eval_const_expr_p4(elem->lhs, ok) -
           eval_const_expr_p4(elem->rhs, ok);
  if (elem->kind == ND_MUL)
  ...
  if (elem->kind == ND_BOR)
    return eval_const_expr_p4(elem->lhs, ok) |
           eval_const_expr_p4(elem->rhs, ok);
  if (elem->kind == ND_BAND)
    return eval_const_expr_p4(elem->lhs, ok) &
           eval_const_expr_p4(elem->rhs, ok);
  if (elem->kind == ND_BXOR)
    return eval_const_expr_p4(elem->lhs, ok) ^
           eval_const_expr_p4(elem->rhs, ok);
  if (elem->kind == ND_SHL)
    return eval_const_expr_p4(elem->lhs, ok)
  ...
  if (elem->kind == ND_BNOT)
    return ~eval_const_expr_p4(elem->lhs, ok);
  if (elem->kind == ND_LNOT)
    return !eval_const_expr_p4(elem->lhs, ok);
  *ok = 0;
  return 0;
}

static void emit_struct_fields(Compiler *cc, StructField *fields, Node **args,
                               int num_args, int *arg_idx, int arg_end,
                               int base_offset, int *emitted) {
  StructField *f;
  for (f = fields; f; f = f->next) {
    ...
    if (f->type->kind == TY_STRUCT) {
      emit_struct_fields(cc, f->type->fields, args, num_args, arg_idx, arg_end,
                         field_abs_offset, emitted);
    } else if (f->type->kind == TY_ARRAY) {
      int j;
      if (f->type->base && (f->type->base->kind == TY_STRUCT ||
                            f->type->base->kind == TY_UNION)) {
        int elem_size = type_size(f->type->base);
        for (j = 0; j < f->type->array_len; j++) {
          int expected_end = field_abs_offset + (j + 1) * elem_size;
          emit_struct_fields(cc, f->type->base->fields, args, num_args, arg_idx,
                             arg_end, field_abs_offset + j * elem_size,
                             emitted);
        ...
        for (j = 0; j < f->type->array_len; j++) {
          if (*arg_idx < arg_end && *arg_idx < num_args) {
            ...
            while (elem && elem->kind == ND_CAST)
              elem = elem->lhs; /* Strip ND_CAST */
            int const_ok = 1;
            ...
            if (!elem) {
              ...
              if (elem_size == 4)
                fprintf(cc->out, "    .long .Lstr_%d\n", elem->str_id);
              else
                fprintf(cc->out, "    .quad .Lstr_%d\n", elem->str_id);
            } else if (elem->kind == ND_ADDR && elem->lhs &&
                       elem->lhs->kind == ND_VAR) {
              if (elem_size == 4)
                fprintf(cc->out, "    .long %s\n", elem->lhs->name);
              else
                fprintf(cc->out, "    .quad %s\n", elem->lhs->name);
            } else if (elem->kind == ND_ADDR && elem->lhs &&
                       elem->lhs->kind == ND_DEREF && elem->lhs->lhs &&
                       elem->lhs->lhs->kind == ND_ADD && elem->lhs->lhs->lhs &&
                       elem->lhs->lhs->lhs->kind == ND_VAR &&
                       elem->lhs->lhs->rhs &&
                       elem->lhs->lhs->rhs->kind == ND_NUM) {
              long long offset = elem->lhs->lhs->rhs->int_val;
              if (elem->lhs->lhs->lhs->type && elem->lhs->lhs->lhs->type->base)
                offset *= type_size(elem->lhs->lhs->lhs->type->base);
              if (elem_size == 4)
                fprintf(cc->out, "    .long %s + %lld\n",
                        elem->lhs->lhs->lhs->name, offset);
              else
                fprintf(cc->out, "    .quad %s + %lld\n",
                        elem->lhs->lhs->lhs->name, offset);
            } else if (elem->kind == ND_ADD && elem->lhs &&
                       elem->lhs->kind == ND_VAR && elem->rhs &&
                       elem->rhs->kind == ND_NUM) {
              long long offset = elem->rhs->int_val;
              if (elem->lhs->type && elem->lhs->type->base)
                offset *= type_size(elem->lhs->type->base);
              if (elem_size == 4)
                fprintf(cc->out, "    .long %s + %lld\n", elem->lhs->name,
                        offset);
              else
              ...
              fprintf(stderr,
                      "STRUCT-ARRAY FALLBACK ZERO: kind=%d, const_ok=%d\n",
                      elem->kind, const_ok);
              if (elem->lhs)
                fprintf(stderr, "  lhs->kind=%d lhs->name=%s\n",
                        elem->lhs->kind,
                        elem->lhs->name ? elem->lhs->name : "none");
              fprintf(cc->out, "    .zero %d\n", elem_size);
            }
          } else {
            fprintf(cc->out, "    .zero %d\n", elem_size);
        ...
        if (!elem) {
          ...
          if (elem_size == 4)
            ...
            fprintf(cc->out, "    .quad %s\n", elem->lhs->name);
                   ...
                   elem->lhs->kind == ND_DEREF && elem->lhs->lhs &&
                   elem->lhs->lhs->kind == ND_ADD && elem->lhs->lhs->lhs &&
                   elem->lhs->lhs->lhs->kind == ND_VAR && elem->lhs->lhs->rhs &&
                   elem->lhs->lhs->rhs->kind == ND_NUM) {
          long long offset = elem->lhs->lhs->rhs->int_val;
          if (elem->lhs->lhs->lhs->type && elem->lhs->lhs->lhs->type->base)
            offset *= type_size(elem->lhs->lhs->lhs->type->base);
          if (elem_size == 4)
            fprintf(cc->out, "    .long %s + %lld\n", elem->lhs->lhs->lhs->name,
                    offset);
          else
            fprintf(cc->out, "    .quad %s + %lld\n", elem->lhs->lhs->lhs->name,
                    offset);
...
static void emit_global_var(Compiler *cc, Node *gvar) {
  ...
  if (gvar->initializer) {
    ...
    if (init && init->kind == ND_NUM) {
      ...
      /* Scalar floating-point initializer: emit IEEE 754 bit pattern */
      if (size == 4) {
        ...
        memcpy(&dbits, &init->f_val, 8);
        fprintf(cc->out, "    .quad %llu\n", dbits);
      }
    } else if (init->kind == ND_ADDR && init->lhs &&
               init->lhs->kind == ND_VAR) {
      if (size == 4)
        fprintf(cc->out, "    .long %s\n", init->lhs->name);
      else
        fprintf(cc->out, "    .quad %s\n", init->lhs->name);
    } else if (init->kind == ND_ADDR && init->lhs &&
               init->lhs->kind == ND_DEREF && init->lhs->lhs &&
               init->lhs->lhs->kind == ND_ADD && init->lhs->lhs->lhs &&
               init->lhs->lhs->lhs->kind == ND_VAR && init->lhs->lhs->rhs &&
               init->lhs->lhs->rhs->kind == ND_NUM) {
      long long offset = init->lhs->lhs->rhs->int_val;
      if (init->lhs->lhs->lhs->type && init->lhs->lhs->lhs->type->base)
        offset *= type_size(init->lhs->lhs->lhs->type->base);
      if (size == 4)
        fprintf(cc->out, "    .long %s + %lld\n", init->lhs->lhs->lhs->name,
                offset);
      else
        fprintf(cc->out, "    .quad %s + %lld\n", init->lhs->lhs->lhs->name,
                offset);
    } else if (init->kind == ND_ADD && init->lhs && init->lhs->kind == ND_VAR &&
               init->rhs && init->rhs->kind == ND_NUM) {
      long long offset = init->rhs->int_val;
      if (init->lhs->type && init->lhs->type->base)
        offset *= type_size(init->lhs->type->base);
      if (size == 4)
      ...
      if (gvar->type && gvar->type->kind == TY_STRUCT) {
        int arg_idx = 0;
        emit_struct_fields(cc, gvar->type->fields, init->args, init->num_args,
                           &arg_idx, init->num_args, 0, &emitted);
      } else if (gvar->type && gvar->type->kind == TY_ARRAY &&
                 gvar->type->base &&
                 (gvar->type->base->kind == TY_STRUCT ||
                  gvar->type->base->kind == TY_UNION)) {
        int i;
        int arg_idx = 0;
        ...
        for (i = 0; i < gvar->type->array_len; i++) {
          ...
          if (arg_end > init->num_args)
            arg_end = init->num_args;
          emit_struct_fields(cc, gvar->type->base->fields, init->args,
                             init->num_args, &arg_idx, arg_end, i * elem_size,
                             &emitted);
          if (emitted < expected_end) {
            fprintf(cc->out, "    .zero %d\n", expected_end - emitted);
            emitted = expected_end;
        ...
        int i;
        int elem_size = 1;
        if (gvar->type && gvar->type->base)
          elem_size = type_size(gvar->type->base);
        for (i = 0; i < init->num_args; i++) {
          Node *elem = init->args[i];
          if (!elem) {
            fprintf(cc->out, "    .zero %d\n", elem_size);
          } else if (elem->kind == ND_FLIT) {
            /* Positive float/double literal */
            if (elem_size == 4) {
              ...
              unsigned long long dbits;
              memcpy(&dbits, &elem->f_val, 8);
              fprintf(cc->out, "    .quad %llu\n", dbits);
            }
          } else if (elem->kind == ND_NEG && elem->lhs && elem->lhs->kind == ND_FLIT) {
            /* Negative float/double literal: -X.Y */
            double neg_val = -(elem->lhs->f_val);
            if (elem_size == 4) {
              float fv = (float)neg_val;
              unsigned int fbits;
              memcpy(&fbits, &fv, 4);
              fprintf(cc->out, "    .long %u\n", fbits);
            } else {
              unsigned long long dbits;
              memcpy(&dbits, &neg_val, 8);
              fprintf(cc->out, "    .quad %llu\n", dbits);
            }
          } else {
            /* Integer and other const expressions */
            int const_ok = 1;
            long long const_val = eval_const_expr_p4(elem, &const_ok);
            if (const_ok) {
              if (elem_size == 1)
    ...
    if (node->lhs && node->lhs->kind == ND_NUM) {
      ...
      if (node->type && !node_type_unsigned(node)) {
        if (node->type->size == 4)
          ...
          res = (long long)(char)res;
        else if (node->type->size == 2)
          res = (long long)(short)res;
      } else if (node->type && node_type_unsigned(node)) {
        if (node->type->size == 4)
          res = (long long)(unsigned int)res;
        else if (node->type->size == 1)
          res = (long long)(unsigned char)res;
        else if (node->type->size == 2)
          res = (long long)(unsigned short)res;
      }
      node->kind = ND_NUM;
      node->int_val = res;
      node->lhs = 0;
    ...
    if (node->lhs && node->rhs && node->lhs->kind == ND_NUM &&
      ...
      if (is_unsigned && node->lhs->type) {
        if (node->lhs->type->size == 4) {
          ...
          u1 = (unsigned short)v1;
          u2 = (unsigned short)v2;
        }
      }
...
void codegen_program(Compiler *cc, Node *prog) {
  ...
  /* Deduplicate tentative definitions: keep only one instance (prioritizing
   * initializers) */
  for (i = 0; i < cc->num_globals; i++) {
    int j;
    Node *g1 = cc->globals[i];
    if (!g1 || g1->kind != ND_GLOBAL_VAR)
      continue;

    for (j = i + 1; j < cc->num_globals; j++) {
      Node *g2 = cc->globals[j];
      if (!g2 || g2->kind != ND_GLOBAL_VAR)
        continue;
...
static void ir_pm_run(ir_pass_manager_t *pm, ir_module_t *mod) {
    ...
    for (p = 0; p < pm->count; p++) {
        ...
        /* Emit per-pass telemetry to Gods Eye (no-op if ZCC_EMIT_TELEMETRY unset) */
        ir_telem_pass(pm->passes[p].name, mod->func_count,
                      pass_before, pass_after, pass_deleted, pass_modified);
    ...
    /* Emit compilation summary telemetry */
    {
        const char *names[IR_PM_MAX_PASSES];
        int i;
        for (i = 0; i < pm->count; i++)
            names[i] = pm->passes[i].name;
        ir_telem_summary(mod->func_count,
...
void ir_pm_run_default(void *mod_ptr, int verbose) {
    ...
    /* Initialize telemetry (no-op if ZCC_EMIT_TELEMETRY unset) */
    ir_telem_init();
