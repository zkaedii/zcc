    ...
    function refreshEntityAnnotationUi() {
      ...
      if (ui.entityAnnotationStatus) {
        ...
        ui.entityAnnotationStatus.textContent = `Entity ${selectedEntityId} Â· ${total} annotation${total === 1 ? "" : "s"} saved.`;
      }
    }
    ...
    function refreshStageHud() {
      ...
      ui.stageHud.textContent = parts.join("  Â·  ");
    }
    ...
    function refreshScatter() {
      ...
      if (ui.scatterTrend && ui.scatterTrend.checked && pts.length > 1) {
        ...
        ui.scatterStats.textContent = `n=${pts.length}  Xâˆˆ[${Math.min(...xVals).toFixed(2)},${Math.max(...xVals).toFixed(2)}]  Yâˆˆ[${Math.min(...yVals).toFixed(2)},${Math.max(...yVals).toFixed(2)}]`;
      }
    ...
    function refreshDiffMatrix() {
      ...
      if (svgEl) {
        svgEl.addEventListener("click", (ev) => {
          ...
          renderCurrent();
          showToast(`Loaded "${a[0]}" â†’ preview, "${b[0]}" â†’ current.`, "ok");
        });
      }
      if (ui.diffMatrixStatus) ui.diffMatrixStatus.textContent = `${n}Ã— snapshots, ch=${ch}, max ${metric}=${mx.toFixed(3)}`;
    }
    ...
    function refreshFrameStrip() {
      ...
      if (ui.frameStripStatus) ui.frameStripStatus.textContent = `${count} frames Â· ${span.toFixed(2)}s span Â· tâ‚€=${start.toFixed(2)}`;
    }
    ...
    function refreshMacroRecorderUi() {
      ...
      if (ui.btnMacroRecToggle) ui.btnMacroRecToggle.textContent = macroRecording ? "â–  Recording" : "â— Record";
      if (ui.macroRecorderStatus) ui.macroRecorderStatus.textContent = `${macroRecorderLines.length} line${macroRecorderLines.length === 1 ? "" : "s"} Â· ${macroRecording ? "recording" : "idle"}`;
    }
    ...
    function showDraftBarIfAny() {
      ...
      ui.draftHint.innerHTML = `<span class="small" style="color:var(--muted)">Text draft</span> Â· saved ${escapeHtml(label)} <button type="button" id="btnApplyDraft">Apply</button> <button type="button" id="btnClearDraft">Clear</button>`;
    }
    ...
    function updateBonusStatus() {
      ...
      const psel = Number.isFinite(selectedEntityId) ? `#${selectedEntityId}` : "â€”";
      ...
      ui.bonusStatus.textContent = bits.join("  Â·  ");
    }
    ...
    function toggleCheckpointPlayer() {
      ...
      const dwell = numInput(ui.checkpointDwell, 80, 5000, 500);
      checkpointPlayer = setTimeout(tick, dwell);
      if (ui.btnCheckpointPlay) ui.btnCheckpointPlay.textContent = "â–  CP";
    ...
    function emitRuntimeHtml(irState) {
      ...
      return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>SVG42 Runtime</title><style>body{margin:0;background:#060b12}#v{width:100vw;height:100vh;display:block}</style></head>
...
function frame(ts){
  ...
  document.getElementById('v').innerHTML='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 '+w+' '+h+'" width="100%" height="100%"><rect width="'+w+'" height="'+h+'" fill="#080d14"/>'+body+'</svg>';
...
<\/script></body></html>`;
    ...
    function renderCurrent() {
      ...
      if (abCompare && previewIr && ir) {
        ...
        const hA = !swap && heatOn
          ? ` + Î´(${escapeHtml(ch)}, ${escapeHtml(ui.deltaMode.value)}) @current`
          : (swap && heatOn ? ` + Î´@checkpoint` : "");
        const hB = !swap && heatB && heatOn
          ? ` + Î´@checkpoint pos`
          : (swap && heatOn ? ` + Î´@current` : "");
        ui.stage.innerHTML = [
          '<div class="ab-wrap">',
    ...
    function applyTheme(name) {
      const valid = ["default", "midnight", "lab"];
      const next = valid.includes(name) ? name : "default";
      if (next === "default") {
    ...
    function startTimeline() {
      ...
      timelineLastTs = 0;
      if (ui.btnTimelinePlay) ui.btnTimelinePlay.textContent = "â¸ Pause";
      const tick = (ts) => {
        if (!timelinePlaying) return;
    ...
    function stopTimeline() {
      ...
      timelineRafId = 0;
      if (ui.btnTimelinePlay) ui.btnTimelinePlay.textContent = "â–¶ Play";
    }
    ...
    function refreshChannelAtlas() {
      ...
      activeIr.schema.forEach((spec, idx) => {
        ...
        const fltText = flt ? `Â· ${flt.lo.toFixed(2)}â€¦${flt.hi.toFixed(2)} <span class="clearx" data-action="clear">âœ•</span>` : "";
        card.innerHTML = `<div class="lbl"><span>${idx + 1}</span><b>${escapeHtml(ch)}</b><span>${spec.mode}</span></div><div class="hist">${histogramSvg(hist, color)}${brushHtml}</div><div class="meta"><span>Î¼=${hist.mean.toFixed(2)} Ïƒ=${hist.std.toFixed(2)} [${hist.min.toFixed(2)},${hist.max.toFixed(2)}]</span><span>${fltText}</span></div>`;
    ...
    function updateAtlasFilterStatus() {
      ...
      const parts = [];
      for (const [ch, r] of atlasFilters) parts.push(`${ch} âˆˆ [${r.lo.toFixed(2)}, ${r.hi.toFixed(2)}]`);
      const activeIr = previewIr || ir;
      const ids = atlasMatchingIds(activeIr);
      const total = activeIr ? activeIr.entities.length : 0;
      const pct = total ? ((ids.length / total) * 100).toFixed(1) : "0.0";
      ui.atlasFilterStatus.textContent = `${parts.join(" âˆ§ ")} Â· ${ids.length}/${total} (${pct}%)`;
    }
    ...
    function bindAtlasBrush() {
      ...
      window.addEventListener("mouseup", (ev) => {
        ...
        refreshChannelAtlas();
        renderCurrent();
        showToast(`Filter ${ch} âˆˆ [${lo.toFixed(2)}, ${hi.toFixed(2)}]`, "ok");
      });
    }
    ...
    function snapshotMetaText() {
      ...
      return `${snapshots.size}/${MAX_SNAPSHOT_COUNT} snapshots Â· ${(sizeBytes / 1024).toFixed(1)}/${(MAX_SNAPSHOT_STORAGE_BYTES / 1024).toFixed(0)} KiB`;
    }
    function refreshSnapshotList() {
      ...
      for (const [name, snap] of sorted) {
        ...
        row.innerHTML = `<div><div class="name">${escapeHtml(name)}</div><div class="sub">${ents} ents Â· ${cps} cp Â· ${escapeHtml((snap.savedAt || "").slice(0, 19))}</div></div>`;
    ...
    function buildHelpDrawerHtml() {
      return `
        ...
        <table>
          ...
          <tr><td><code>quantize</code></td><td>quantize rot 24</td><td>steps â‰¥ 1</td></tr>
        ...
        <table>
          ...
          <tr><td><code>quantize</code></td><td>quantize ch steps</td><td>steps â‰¥ 1</td></tr>
          ...
          <tr><td><code>invert</code></td><td>invert ch</td><td>minâ†”max around span</td></tr>
    ...
    function buildCheatsheetHtml() {
      ...
      return `<span class="close-x" id="cheatClose" title="Close">âœ•</span><h3>Hotkeys</h3><dl>${dl}</dl><p class="small" style="color:var(--muted);margin:4px 0 0">Hotkeys are ignored when typing in inputs/textareas.</p>`;
    }
    ...
    function runPredicateSearch(expr) {
      ...
      try {
        ...
        ui.searchResults.textContent = ids.length ? `${preview}${ids.length > 30 ? `, â€¦ (+${ids.length - 30})` : ""}` : "(none)";
        return ids;
      } catch (e) {
    ...
    function dryRunPhase2Line() {
      ...
      try {
        ...
        const top = changes.slice(0, 6).map((c) =>
          `${c.channel}: touched ${c.touched}, |Î”|=${c.meanAbsDelta.toFixed(3)} dÎ¼=${c.dMean.toFixed(3)} [${c.rangeBefore[0].toFixed(2)},${c.rangeBefore[1].toFixed(2)}] â†’ [${c.rangeAfter[0].toFixed(2)},${c.rangeAfter[1].toFixed(2)}]`
        ).join("\n");
    ...
    function renameChannel() {
      ...
      try {
        ...
        ui.renameStatus.innerHTML = `<span class="ok">Renamed ${oldName} â†’ ${newName}.</span>`;
        showToast(`Renamed channel ${oldName} â†’ ${newName}.`, "ok");
      } catch (e) {
    ...
    function refreshPinToolkit() {
      ...
      if (ui.pinToolkitCount) ui.pinToolkitCount.textContent = `${pinnedEntityIds.size} pinned Â· ${activeIr ? activeIr.entities.length : 0} entities`;
    }
    ...
    function exportFrameSequenceHtml() {
      ...
      const cards = frames.map((f, idx) => `<figure class="card"><figcaption>frame ${idx} Â· t=${f.t.toFixed(2)}s</figcaption>${f.svg}</figure>`).join("\n");
      const html = `<!doctype html><html><head><meta charset="utf-8"><title>SVG42 frame sequence</title>
<style>body{margin:0;background:#06101c;color:#dde7ff;font-family:ui-sans-serif,system-ui,sans-serif}h1{padding:14px 18px;margin:0;border-bottom:1px solid #20304a}main{display:grid;grid-template-columns:repeat(auto-fill,minmax(360px,1fr));gap:12px;padding:12px}figure{margin:0;background:#0f1620;border:1px solid #20304a;border-radius:8px;overflow:hidden}figcaption{padding:6px 10px;font-size:0.78rem;color:#8ca2c7;border-bottom:1px solid #20304a}svg{display:block;width:100%;height:auto}</style></head>
<body><h1>SVG42 frame sequence â€” ${count} frames @ ${step}s step</h1><main>${cards}</main></body></html>`;
      ...
      ui.draftHint.addEventListener("click", (ev) => {
        ...
        if (ev.target.id === "btnApplyDraft") {
          ...
          ui.compileStatus.innerHTML = ir
            ...
            : "<span class=\"warn\">Draft applied; compile failed â€” check matrix/DSL.</span>";
          showDraftBarIfAny();
        }
      });
    }
    ...
    schema = parseDsl(ui.builderDsl.value);
    updatePlotChannelOptions();
    if (!tryApplyHashFromUrl()) compileAndReport();
    else { /* compile already ran via tryApplyHashFromUrl */ }
    updateCheckpointUi();
    updatePinGroupUi();
    refreshSnapshotList();
    refreshCustomMacroUi();
