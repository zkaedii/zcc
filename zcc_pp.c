...
int64_t       node_int_val(struct Node *n);
...
int64_t       node_case_val(struct Node *n);       /* ND_CASE: case constant */
...
void run_all_passes(struct Function *fn, struct PassResult *result, const char *profile_path);
...
static Type *parse_struct_or_union(Compiler *cc, int is_union) {
...

    {
        ...
        while (cc->tk != TK_RBRACE) {
            ...
            if (fname[0] == '\0' && (ftype->kind == TY_STRUCT || ftype->kind == TY_UNION) && ftype->tag[0] == '\0') {
                ...
                while (anon_field) {
                    StructField *new_field = (StructField *)cc_alloc(cc, sizeof(StructField));
                ...
                if (cc->tk == TK_SEMI) {
                    ...
                    if (cc->tk == TK_RBRACE || cc->tk == TK_EOF) break;
                    next_token(cc);
                }
                continue;
            }
            ...
            if (is_union) {
                ...
                if (has_colon) {
                    if (bitfield_width == 0 || type_size(ftype) != current_bitfield_type_size || (current_bit_offset + bitfield_width > type_size(ftype) * 8)) {
                        if (current_bit_offset > 0) {
                            offset += current_bitfield_type_size;
                        }
                        if (falign > 1) {
            ...
            while (cc->tk == TK_COMMA) {
                ...
                if (is_union) {
                    ...
                    if (has_colon2) {
                        if (bitfield_width2 == 0 || type_size(ftype2) != current_bitfield_type_size || (current_bit_offset + bitfield_width2 > type_size(ftype2) * 8)) {
                            if (current_bit_offset > 0) {
                                offset += current_bitfield_type_size;
                            }
                            if (falign2 > 1) {
...
Node *parse_primary(Compiler *cc) {
    ...
    {
        char buf[512];
        sprintf(buf, "unexpected token %d ('%s') in expression", cc->tk, cc->tk_text);
        error(cc, buf);
        next_token(cc);
